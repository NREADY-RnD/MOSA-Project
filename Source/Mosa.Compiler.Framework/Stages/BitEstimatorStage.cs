...
namespace Mosa.Compiler.Framework.Stages
{
	...
	/// </summary>
	public sealed class BitEstimatorStage : BaseMethodCompilerStage
	{
		...
		private struct Value
		{
			...
			public bool IsIndeterminate { get { return IsMaxValueIndeterminate && IsMinValueIndeterminate && BitsClear == 0 && BitsSet == 0; } }
		...
		private Counter InstructionUpdateCount = new Counter("BitEstimatorStage.InstructionUpdate");
		...
		private bool Evaluate(Operand virtualRegister)
		{
			...
			if (virtualRegister.Uses.Count == 0)
			{
			...
			if (virtualRegister.IsR || virtualRegister.Definitions.Count != 1)
			{
				Values[index].SetIndeterminate();
				return true;
			}
		...
		private void ShiftRight64(InstructionNode node)
		{
			...
			if (value1.AreAll64BitsKnown && (value1.MaxValue & uint.MaxValue) == 0)
			{
		...
		private void LoadZeroExtend8x32(InstructionNode node)
		{
			...
			Values[index].BitsClear = ulong.MaxValue & ~(ulong)(byte.MaxValue);
			Values[index].IsEvaluated = true;
		}
		...
		private void LoadZeroExtend16x32(InstructionNode node)
		{
			...
			Values[index].IsMinValueIndeterminate = false;
			Values[index].BitsSet = 0;
			Values[index].BitsClear = ulong.MaxValue & ~(ulong)(ushort.MaxValue);
			Values[index].IsEvaluated = true;
		}
		...
		private void CompareInt32x32(InstructionNode node)
		{
			...
			if (conditionCode == ConditionCode.Equal || conditionCode == ConditionCode.NotEqual)
			{
				// assume Equal for now, will flip result later for NotEqual later
				if (value1.AreLower32BitsKnown && value2.AreLower32BitsKnown)
				{
					hasResult = value1.MaxValue == value2.MaxValue;
					result = true;
				}
				else
				{
					// if MaxValue(s) == MinValues(s) and nothing is indeterminate, then must be equal
					if (!value1.IsMaxValueIndeterminate && !value1.IsMinValueIndeterminate && !value2.IsMaxValueIndeterminate && !value2.IsMinValueIndeterminate && value1.MaxValue == value2.MaxValue && value1.MinValue == value2.MinValue)
					{
						hasResult = true;
						result = true;
					}
					...
					if (((value1.BitsSet & value2.BitsClear) != 0) || ((value2.BitsSet & value1.BitsClear) != 0))
					{
						hasResult = true;
						result = false;
					}
				...
				// flip the result for NotEqual
				if (conditionCode == ConditionCode.NotEqual && hasResult)
				{
					result = !result;
				}
			}
			...
			Values[index].BitsSet = result ? 1u : 0u;
			Values[index].BitsClear = ~(result ? 1u : 0u);
			Values[index].IsEvaluated = true;
		}
	}
}
